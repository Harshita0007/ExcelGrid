...
const FileManagerModal = ({ isOpen, onClose, onSave, onImport, onRefresh, data }) => {
...
const ExcelGrid = ({
  ...
  const [colWidth, setColWidth] = useState(() => Array(initialColumns).fill(120));
      ...
      if (gridWrapperRef.current) {
        ...
        setColWidth((prev) => {
          ...
          if (prev.length < currentColumns) {
            return [...prev, ...Array(currentColumns - prev.length).fill(newColWidth)];
          }
  ...
  const pagedRows = sortedData.slice((currentPage - 1) * rowsPerPage, currentPage * rowsPerPage);
  ...
  const handleCopy = () => {
    let text = "";
    if (selectedRange) {
      const { minRow, maxRow, minCol, maxCol } = getSelectedRectangle(selectedRange);
      const rows = [];
    ...
    if (selectedRange) {
      ...
      for (let r = minRow; r <= maxRow; r++) {
        ...
        for (let c = minCol; c <= maxCol; c++) {
          row.push(sortedData[r]?.cells[c] ?? "");
          // Clear cell
          const gridRow = newData.findIndex(rowObj => rowObj.id === sortedData[r].id);
  ...
  const handlePaste = async () => {
    ...
    let startRow = selectedRange ? Math.min(selectedRange.start.row, selectedRange.end.row) : focusedCell.row;
    let startCol = selectedRange ? Math.min(selectedRange.start.col, selectedRange.end.col) : focusedCell.col;
    let newData = [...data];
  ...
  const handleCellMouseDown = (rowIndex, colIndex, e) => {
    if (e.shiftKey && focusedCell) {
      setSelectedRange({ start: focusedCell, end: { row: rowIndex, col: colIndex } });
    } else {
  ...
  const handleCellMouseEnter = (rowIndex, colIndex) => {
    if (mouseDownRef.current && selectedRange) {
      setSelectedRange((range) => ({ ...range, end: { row: rowIndex, col: colIndex } }));
    }
  };
      ...
      if (e.shiftKey && ["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(e.key)) {
        ...
        switch (e.key) {
          case "ArrowUp":
            ...
            break;
          case "ArrowDown":
            newEnd = { row: Math.min(data.length - 1, newEnd.row + 1), col: newEnd.col };
            break;
          case "ArrowLeft":
            newEnd = { row: newEnd.row, col: Math.max(0, newEnd.col - 1) };
            break;
          case "ArrowRight":
            newEnd = { row: newEnd.row, col: Math.min(data[0].cells.length - 1, newEnd.col + 1) };
            break;
        }
        setSelectedRange((range) => ({ start: range?.start || focusedCell, end: newEnd }));
      }
    };
  ...
  const handleImport = (file) => {
    importFromFile(file, (newData) => {
      ...
      const gridData = createInitialGridData(newData, initialRows, initialColumns);
      setData(gridData);
  ...
  const nonEmptyRowCount = data.filter(row => row.cells.some(cell => String(cell).trim() !== "")).length;

  // Count only columns with at least one non-empty cell
  const nonEmptyColCount = (data[0]?.cells || []).filter((_, colIdx) => data.some(row => String(row.cells[colIdx] ?? '').trim() !== '')).length;
    ...
    >
      <div
        ...
        style={{
          ...
          borderRadius: "16px",
          boxShadow: darkMode ? "0 2px 24px rgba(30,30,50,0.25)" : "0 2px 24px rgba(0,0,0,0.10)",
          display: "flex",
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            zIndex: 10,
            padding: "8px 16px",
            background: darkMode ? "linear-gradient(90deg, #22263a 0%, #3a3a5a 100%)" : "linear-gradient(90deg, #6f42c1 0%, #8e54e9 100%)",
            color: "#fff",
            border: "none",
            borderRadius: "18px",
            cursor: "pointer",
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", margin: 0, padding: "40px 40px 0 40px" }}>
          <h1 style={{ fontWeight: 700, fontSize: "2.5rem", margin: 0, display: "flex", alignItems: "center", gap: 12 }}>
            <span role="img" aria-label="spreadsheet">🧮</span> Excel-like Grid Component
          </h1>
          <div style={{ margin: "24px 0 16px 0", display: "flex", flexWrap: "wrap", gap: "0" }}>
            <button style={toolbarButtonStyle} onClick={handleAddRow}>➕ Add Row</button>
            <button style={toolbarButtonStyle} onClick={handleAddColumn}>➕ Add Column</button>
            <button style={toolbarButtonStyle} onClick={handleDeleteRow}>🗑️ Delete Row</button>
            <button style={toolbarButtonStyle} onClick={handleDeleteColumn}>🗑️ Delete Column</button>
            <button style={toolbarButtonStyle} onClick={handleClearAll}>🧹 Clear All</button>
            <button style={{...toolbarButtonStyle, opacity: historyIndex <= 0 ? 0.5 : 1, cursor: historyIndex <= 0 ? 'not-allowed' : 'pointer'}} onClick={undo} disabled={historyIndex <= 0}>↩️ Undo</button>
            <button style={{...toolbarButtonStyle, opacity: historyIndex >= history.length - 1 ? 0.5 : 1, cursor: historyIndex >= history.length - 1 ? 'not-allowed' : 'pointer'}} onClick={redo} disabled={historyIndex >= history.length - 1}>↪️ Redo</button>
          </div>
        </div>
        <div
          ref={gridWrapperRef}
        ...
        >
          ...
          >
            <thead>
              <tr>
                <th
                  style={{
                    ...
                    borderRight: darkMode ? "2px solid #333" : "2px solid #e0e0e0",
                    boxShadow: darkMode ? "2px 0 6px -2px rgba(30,30,50,0.10)" : "2px 0 6px -2px rgba(0,0,0,0.04)",
                  }}
                ></th>
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{
                        ...
                        zIndex: 10,
                        userSelect: 'none',
                        background: resizingCol === colIndex ? (darkMode ? 'rgba(33,134,235,0.18)' : 'rgba(33,134,235,0.12)') : 'transparent',
              ...
              {showFilterRow && (
                <tr>
                  ...
                  >
                    <button onClick={() => setShowFilterRow(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, color: darkMode ? '#aaa' : '#888' }} title="Collapse filter row">✖️</button>
                  </th>
                  {columnHeaders.map((_, colIndex) => (
                    <th
                      ...
                      style={{
                        ...
                        backgroundColor: darkMode ? "#232336" : "#f3f6fa",
                        color: darkMode ? "#e5e7eb" : "#222",
                        border: darkMode ? "1px solid #333" : "1px solid #e3e8ee",
                        position: "sticky",
                        top: 48,
                        zIndex: 5,
                        height: "60px",
                        boxShadow: darkMode ? "0 2px 6px -2px rgba(30,30,50,0.13)" : "0 2px 6px -2px rgba(0,0,0,0.07)",
                        borderBottom: darkMode ? "2px solid #333" : "2px solid #e0e0e0",
                      }}
                    >
                      <div
                      ...
                      >
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            setFilters((prev) => ({
                              ...
                              [colIndex]: {
                                ...(prev[colIndex] || {}),
                                type,
                                value: type === "notEmpty" || type === "empty" ? "1" : "",
                              },
                            }));
                          }}
                          onClick={(e) => e.stopPropagation()}
                          ...
                          style={{
                            ...
                            fontSize: "13px",
                            padding: "4px 8px",
                            border: darkMode ? "1px solid #444" : "1px solid #bfc9d9",
                            borderRadius: "8px",
                            background: darkMode ? "#232336" : "#fff",
                            color: darkMode ? "#e5e7eb" : "#222",
                            boxShadow: darkMode ? "0 1px 2px rgba(30,30,50,0.10)" : "0 1px 2px rgba(0,0,0,0.03)",
                            outline: "none",
                        ...
                        <input
                          ...
                          style={{
                            ...
                            outline: "none",
                            transition: "border 0.2s",
                            display: filters[colIndex]?.type === "notEmpty" || filters[colIndex]?.type === "empty" ? "none" : "block",
                          }}
                        />
                      </div>
                    </th>
                  ...
                  <tr
                    ...
                    style={{
                      backgroundColor: darkMode
                        ? globalRowIndex % 2 === 0 ? "#232336" : "#282846"
                        : globalRowIndex % 2 === 0 ? "#f9f9fb" : "#fff",
                      color: darkMode ? "#e5e7eb" : "#222",
                      transition: "background 0.2s, color 0.2s",
                    }}
                  >
                    <th
                      style={{
                        backgroundColor: darkMode ? "#232336" : focusedCell.row === globalRowIndex ? "#f8f9fa" : "#f1f1f1",
                        color: darkMode ? "#e5e7eb" : "#222",
                        border: darkMode ? "1px solid #333" : "1px solid #ddd",
                        textAlign: "center",
                        position: "sticky",
                    ...
                    {row.cells.map((cell, colIndex) => {
                      const isFocused = focusedCell.row === globalRowIndex && focusedCell.col === colIndex;
                      const isEditing = editingCell?.rowIndex === globalRowIndex && editingCell?.col === colIndex;
                      const isSelected = cellIsInSelectedRange(globalRowIndex, colIndex, selectedRange);
                      return (
                        <td
                          key={colIndex}
                          onClick={(e) => handleCellClick(globalRowIndex, colIndex, e)}
                          onMouseDown={(e) => handleCellMouseDown(globalRowIndex, colIndex, e)}
                          onMouseEnter={() => handleCellMouseEnter(globalRowIndex, colIndex)}
                          onDoubleClick={() => handleCellDoubleClick(globalRowIndex, colIndex)}
                          onContextMenu={(e) => handleContextMenu(e, globalRowIndex, colIndex)}
                          style={{
                            width: `${colWidth[colIndex]}px`,
                            padding: "10px 8px",
                            cursor: "pointer",
                            backgroundColor: isFocused
                              ? darkMode ? "#2d3a4a" : "#eaf3fb"
                              : isSelected
                                ? darkMode ? "#1e293b" : "#dbeafe"
                                : "inherit",
                            color: darkMode ? "#e5e7eb" : "#222",
                            borderTop: isSelected && (!cellIsInSelectedRange(globalRowIndex - 1, colIndex, selectedRange)) ? (darkMode ? "2px solid #38bdf8" : "2px solid #2186eb") : (darkMode ? "1px solid #333" : "1px solid #ccc"),
                            borderBottom: isSelected && (!cellIsInSelectedRange(globalRowIndex + 1, colIndex, selectedRange)) ? (darkMode ? "2px solid #38bdf8" : "2px solid #2186eb") : (darkMode ? "1px solid #333" : "1px solid #ccc"),
                            borderLeft: isSelected && (!cellIsInSelectedRange(globalRowIndex, colIndex - 1, selectedRange)) ? (darkMode ? "2px solid #38bdf8" : "2px solid #2186eb") : (darkMode ? "1px solid #333" : "1px solid #ccc"),
                            borderRight: isSelected && (!cellIsInSelectedRange(globalRowIndex, colIndex + 1, selectedRange)) ? (darkMode ? "2px solid #38bdf8" : "2px solid #2186eb") : (darkMode ? "1px solid #333" : "1px solid #ccc"),
                            boxShadow: isFocused ? (darkMode ? "0 0 0 2px #38bdf8 inset" : "0 0 0 2px #2186eb inset") : undefined,
                            fontSize: "1rem",
                            transition: "background 0.2s, outline 0.2s, border 0.2s, color 0.2s",
                            overflow: "hidden",
            ...
            <tfoot>
              <tr>
                <td colSpan={columnHeaders.length + 1} style={{ padding: '16px 0', background: '#fff', textAlign: 'center', border: 'none' }}>
                  <div style={{ display: 'inline-flex', justifyContent: 'center', alignItems: 'center', gap: 16 }}>
                    <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} style={{ padding: '8px 16px', borderRadius: 6, border: '1px solid #ccc', background: currentPage === 1 ? '#eee' : '#fff', cursor: currentPage === 1 ? 'not-allowed' : 'pointer' }}>Prev</button>
                    <span style={{ fontWeight: 500, fontSize: '1.1em' }}>Page {currentPage} of {totalPages}</span>
                    <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} style={{ padding: '8px 16px', borderRadius: 6, border: '1px solid #ccc', background: currentPage === totalPages ? '#eee' : '#fff', cursor: currentPage === totalPages ? 'not-allowed' : 'pointer' }}>Next</button>
                  </div>
                </td>
              </tr>
            </tfoot>
        ...
        >
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span role="img" aria-label="rows" style={{ fontSize: '1.1em', opacity: 0.85 }}>🔢</span>
            Rows: <span style={{ fontVariantNumeric: 'tabular-nums', marginLeft: 2 }}>{nonEmptyRowCount}</span>
          </span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span role="img" aria-label="columns" style={{ fontSize: '1.1em', opacity: 0.85 }}>📊</span>
            Columns: <span style={{ fontVariantNumeric: 'tabular-nums', marginLeft: 2 }}>{nonEmptyColCount}</span>
          </span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span role="img" aria-label="selected cell" style={{ fontSize: '1.1em', opacity: 0.85 }}>🔲</span>
            Selected: <span style={{ fontVariantNumeric: 'tabular-nums', marginLeft: 2 }}>{String.fromCharCode(65 + (focusedCell.col ?? 0))}{(focusedCell.row + 1).toString().padStart(2, '0')}</span>
          </span>
        </div>
      </div>
    </div>
